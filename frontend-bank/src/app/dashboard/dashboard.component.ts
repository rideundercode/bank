...
export class DashboardComponent implements OnInit {
  ...
  loadCustomerAccounts(): void {
    this.customerService.getCustomers().subscribe(customers => {
      ...
      forkJoin(accountCounts).subscribe(counts => {
        this.renderBarChart(barChartLabels, counts);
      });
    });
  }
  ...
  renderPieChart(data: number[]): void {
    new Chart("pieChart", {
      type: 'pie',
      data: {
  ...
  renderDoughnutChart(data: number[]): void {
  ...
  renderLineChart(labels: string[], data: number[]): void {
    new Chart("lineChart", {
      type: 'line',
      data: {
        labels: labels,
